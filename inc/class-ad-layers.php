...
if ( ! class_exists( 'Ad_Layers\Ad_Layers' ) ) :
	...
	class Ad_Layers extends Ad_Layers_Singleton {
		...
		public function setup() {
			...
			add_action( 'wp_head', [ $this, 'set_active_ad_layer' ], 1 );
		}
		 ...
		 *
		 * @access private
		 * @param string $object_type targeting objects.
		...
		public function set_active_ad_layer() {
			...
			$ad_layer = apply_filters( 'ad_layers_active_ad_layer', [], $queried_object );
			if ( ! empty( $ad_layer ) ) {
		...
		public function get_page_types() {
			if ( empty( $this->page_types ) ) {
				...
				$single_post_types = apply_filters( 'ad_layers_ad_server_single_post_types', wp_list_filter( get_post_types( [ 'public' => true ], 'objects' ), [ 'label' => false ], 'NOT' ) );
				...
				$archived_post_types = apply_filters( 'ad_layers_ad_server_archived_post_types', wp_list_filter( get_post_types( [ 'has_archive' => true ], 'objects' ), [ 'label' => false ], 'NOT' ) );
				...
				// Add taxonomies.
				$taxonomies = apply_filters( 'ad_layers_ad_server_taxonomies', wp_list_filter( get_taxonomies( [ 'public' => true ], 'objects' ), [ 'label' => false ], 'NOT' ) );
				if ( ! empty( $taxonomies ) ) {
					foreach ( $taxonomies as $taxonomy ) {
